...
export async function captureBatchOfPhotos(nextPhoto, capturePhoto) {
  ...
  let photoCount = 0;
  let name;
  // create a header based on the height and width of the image
  csvData += `name,${Array.from({ length: width * height }, (_, i) => `pixel${i}`).join(',')}\n`;

  do {
    name = nextPhoto();
    capturePhoto(pixels); // Capture photo data
    csvData += capturePixelsToCSV(name); // Convert directly without await
